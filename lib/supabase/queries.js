...
// Settings SINGLETON kaydı — çift-satır güvenli.
// id yoksa (ilk kayıt / form id'siz yüklendi) körlemesine INSERT etmek yerine
// DB'den taze bakar; satır varsa onu günceller, yoksa TEK satır oluşturur.
// Dönen kaydın id'si forma yazılmalı ki sonraki kayıtlar update yolunu kullansın.
export async function ayarlariKaydet(supabase, veri) {
  const { id, ...degisiklikler } = veri || {};
  let hedefId = id;
  if (!hedefId) {
    const mevcut = await ayarlariGetir(supabase); // limit(1)
    hedefId = mevcut?.id || null;
  }
  if (hedefId) {
    return ayarlariGuncelle(supabase, hedefId, degisiklikler);
  }
  const { data, error } = await supabase
    .from('settings')
    .insert([degisiklikler])
    .select()
    .maybeSingle();
  if (error) throw error;
  return data;
