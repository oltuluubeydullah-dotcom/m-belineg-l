...
---
_Son güncelleme: v59 — hero teslimat afişleri (main'de CANLI) + WhatsApp şablonları canlıya + talepler/yorumlar sayfalama + ürün-sil görsel temizliği + settings singleton + toplu-yükleme backend'i (branch'te). Bekleyen: (1) sql/21-mobel-arsiv-bucket.sql'i canlı Supabase'de çalıştır (toplu yükleme için), (2) branch'i main'e merge kararı, (3) hukuki [DOLDUR] + harita pini (kullanıcı bilgisi bekliyor), (4) R2 + domain DNS (nameserver bekliyor). Yeni oturum bu dosyadan devam etmeli. by ubivo._
