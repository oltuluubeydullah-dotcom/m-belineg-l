...
3. **Cloudflare R2:** ✅ **CANLI (v62)** — bucket `mobel-medya`, görsel CDN `cdn.mobelinegol.com` (custom domain **Active**). 5 Vercel env (`R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET=mobel-medya`, `NEXT_PUBLIC_R2_PUBLIC_URL=https://cdn.mobelinegol.com`) girildi → R2 aktif. **Tüm mevcut görseller Supabase Storage'dan R2'ye taşındı** (tek-seferlik `/admin/gorsel-tasima` aracı — sonra menüden gizlendi). Yeni yüklemeler cdn'e gidiyor. (Env eksik olursa kod otomatik Supabase Storage'a düşer — güvenli fallback.)
