// ════════════════════════════════════════════════════════════
// gizlilik — CMS'ten içerik (admin düzenler)
// ════════════════════════════════════════════════════════════
...
export async function generateMetadata({ params: { locale } }) {
  const sayfa = await getContentPage('gizlilik', locale);
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://mobelinegol.com';
  const canonicalPath = locale === 'tr' ? '/gizlilik' : `/${locale}/gizlilik`;
  return {
    title: sayfa?.meta_title || sayfa?.title || 'gizlilik',
    description: sayfa?.meta_desc,
    alternates: {
      canonical: `${siteUrl}${canonicalPath}`,
      languages: {
        tr: `${siteUrl}/gizlilik`,
        en: `${siteUrl}/en/gizlilik`,
        de: `${siteUrl}/de/gizlilik`,
      },
    },
  };
}
