// ════════════════════════════════════════════════════════════
// satis-sozlesmesi — CMS'ten içerik (admin düzenler)
// ════════════════════════════════════════════════════════════
...
export async function generateMetadata({ params: { locale } }) {
  const sayfa = await getContentPage('satis-sozlesmesi', locale);
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://mobelinegol.com';
  const canonicalPath = locale === 'tr' ? '/satis-sozlesmesi' : `/${locale}/satis-sozlesmesi`;
  return {
    title: sayfa?.meta_title || sayfa?.title || 'satis-sozlesmesi',
    description: sayfa?.meta_desc,
    alternates: {
      canonical: `${siteUrl}${canonicalPath}`,
      languages: {
        tr: `${siteUrl}/satis-sozlesmesi`,
        en: `${siteUrl}/en/satis-sozlesmesi`,
        de: `${siteUrl}/de/satis-sozlesmesi`,
      },
    },
  };
}
